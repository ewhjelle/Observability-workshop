...
var openTelemetryResourceBuilder = ResourceBuilder.CreateDefault().AddService("serviceOne");
...
{
  string serviceTwoEndpoint = Environment.GetEnvironmentVariable("SERVICE_TWO_ENDPOINT");
  logger.LogInformation("Connecting to: " + serviceTwoEndpoint);
   Thread.Sleep(2000);
...
{
  string serviceTwoEndpoint = Environment.GetEnvironmentVariable("SERVICE_TWO_ENDPOINT");
  logger.LogWarning("The server is not started correctly");
  ...
  logger.LogInformation("Not able to connect to database");
  ...
  logger.LogInformation("Operation pending for user 943");
  ...
  logger.LogInformation("Seccessfully connected");
